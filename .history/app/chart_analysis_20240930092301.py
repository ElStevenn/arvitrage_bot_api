...
class FundingRateChart:
    ...
    async def get_8h_variation(self, period: int):
        ...
        self.df8hdf = pd.DataFrame(candle_stick_data, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume', 'notional'])

        print(df)

        # Calculate variation
        start_price = df['open'].iloc[0]
        regression = df['low'].min()
    ...
    async def set_description(self, regression_8h, volatility_10m):
