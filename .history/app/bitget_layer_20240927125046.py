...
class BitgetService:
    ...
    async def get_candlestick_chart(self, symbol: str, granularity: str, start_time: int = None, end_time: int = None) -> np.ndarray:
            ...
            api_calls = self.calculate_api_calls(start_time, end_time, granularity_ms); print("Api calls -> ", api_calls)

         
            async with aiohttp.ClientSession() as session:
                for call in enumerate(api_calls):
...
async def main_testing():
    ...
    end_time = int(datetime(2024, 9, 20).timestamp() * 1000)
