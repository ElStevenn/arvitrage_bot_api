            ...
            while True:
                async with session.get(base_url, params=params) as response:
                    if response.status == 200:
                        ...
                        # Update the startTime to the last timestamp + 1 to fetch the next batch of data
...
async def main_testing():
    ...
    df = pd.DataFrame(res, columns=['funding_rate', 'datetime', 'datetimev2'])
    df.to_csv('delete_this.csv')
