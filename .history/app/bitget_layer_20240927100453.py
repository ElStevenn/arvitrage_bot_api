...
async def main_testing():
    # Example of how you could calculate a Unix timestamp in milliseconds
    start_time = int(datetime(2023, 1, 1).timestamp() * 1000)  # Example start time in Unix milliseconds
    end_time = int(datetime(2023, 1, 2).timestamp() * 1000)  # Example end time in Unix milliseconds

    # Initialize the BitgetService (assuming BitgetService is your class that has get_candlestick_chart)
    bitget_layer = BitgetService()

    # Example: fetch 1-hour candlestick data for BTCUSDT between the given start and end times
    granularity = Granularity.HOUR_1

    # Call the function with the symbol, granularity, and optional start/end times
    res = await bitget_layer.get_candlestick_chart("BTCUSDT", granularity, str(start_time), str(end_time))
