        ...
        while True:
            async with session.get(base_url, params=params) as response:
                if response.status == 200:
                    ...
                    # Check if the last timestamp exceeds or equals the end time
                    if end_time and last_timestamp >= end_time:
                        break

                    # Check if we have already fetched the required number of candles
                    fetched_candles = len(final_result)
                    if fetched_candles >= total_candles:
                        break
