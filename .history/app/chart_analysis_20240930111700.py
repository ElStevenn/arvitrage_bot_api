...
class FundingRateChart:
    ...
    async def set_analysis(self, period: int) -> dict:
        # Start tasks concurrently where possible
        tasks = [
            ...
            self.market_sentiment(period)
        ]
        h8_variation, h10m_variation, daily_trend, weekly_trend, average_trading_volume, market_sentiment = await asyncio.gather(*tasks)
    ...
    async def market_sentiment(self, period: int) -> Literal[
        ...
        if self.df8h is None:
            await self.get_8h_variation(period)
        if self.df10m is None:
            await self.get_10m_variation(period)
        if self.dfdaily is None:
            await self.get_average_trading_volume(period)
        if self.dfweekly is None:
            await self.get_weekly_trends(period)

        # Get trends and volatility
        daily_trend = await self.get_daily_trend(period)
        weekly_trend = await self.get_weekly_trends(period)
        volatility_index = await self.get_volatility_index()
        average_volume = await self.get_average_trading_volume(period)
