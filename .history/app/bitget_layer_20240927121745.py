...
class BitgetService:
   ...
   async def get_candlestick_chart(self, symbol: str, granularity: str, start_time: int = None, end_time: int = None) -> np.ndarray:
    ...
    params = {
        'symbol': symbol,
        'granularity': granularity,  # Use the correct format like '1H', '1m', etc.
        'productType': 'USDT-FUTURES',
        'limit': 1000
    }
    ...
    if start_time is None:
        start_time = end_time - self.convert_granularity_to_ms(granularity) * 1000  # Default to fetch the last 1000 data points
        ...
        while True:
            ...
            async with session.get(base_url, params=params) as response:
                if response.status == 200:
                    ...
                    new_end_time = earliest_timestamp - self.convert_granularity_to_ms(granularity)
