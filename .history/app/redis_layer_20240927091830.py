...
class RedisService:
    ...
    # FUNDING RATE
    def set_analysis_last_funding(self, symbodef set_analysis_last_funding(self, symbol: str, funding_rate_analysis: FundingRateAnalysis):
    """
    ...
    if not existing_data:
        # If no data exists, create a new structure with the provided funding rate analysis
        new_data = {
            "symbol": symbol,
            "data": [funding_rate_analysis]
        }
    else:
        # Load existing data and append the new analysis to the "data" field
        ...
        if "data" in existing_data and isinstance(existing_data["data"], list):
            if len(existing_data["data"]) >= 500:
                existing_data["data"].pop(0)  # Enforce limit of 500 records
            existing_data["data"].append(funding_rate_analysis)
        else:
            # Handle the case where "data" is not correctly formatted
...
l: str, funding_rate_analysis: FundingRateAnalysis):
        """
        Adds a new analysis or just a new funding rate attached to the symbol.
        {
                "id": uuid,
    ...
    def read_crypto_analysis(self, symbol: str, limit: int = 20):
