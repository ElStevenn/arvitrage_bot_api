...
class BitgetService:
    ...
    async def get_candlestick_chart(self, symbol: str, granularity: Granularity, start_time: int = None, end_time: int = None) -> np.ndarray:
        ...
        params = {
            ...
            'productType': 'USDT-FUTURES',
            'limit': 1000  # The API limit per request
        }
            ...
            while True:
                async with session.get(base_url, params=params) as response:
                    ...
                    if response.status == 200:
                        ...
                        # If we are past the end_time, stop fetching data
                        if end_time and int(last_timestamp) >= end_time:
                            break

                        # Update the startTime for the next API request to continue fetching from the last timestamp
