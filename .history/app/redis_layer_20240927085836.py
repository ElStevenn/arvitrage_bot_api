...
class RedisService:
    ...
    def add_new_crypto(self, symbol: str, whole_analysis_until_now: List[dict], period: Literal['8h', '4h']):
        ...
        """
# Check if the crypto symbol already exists in Redis
    crypto_data = self._r.hget("all_crypto_analysis", symbol)

    # If the crypto already exists, do nothing
    if crypto_data:
        return  # Early exit if the symbol already exists
    ...
    # Store the new crypto entry in Redis
    self._r.hset("all_crypto_analysis", symbol, json.dumps(new_entry))
        ...
        if crypto_list:
            ...
            # Initialize an empty list if no list exists
            crypto_list = []
