...
def add_new_crypto(self, symbol: str, whole_analysis_until_now: List[dict]):
    ...
    """
    # Check if the crypto already exists in the analysis data
    ...
    if all_cryptos:
        ...
        if len(all_cryptos) >= 500:
            all_cryptos.pop(0)
        all_cryptos.append(whole_analysis_until_now)  # Append new analysis
    else:
        # If the crypto doesn't exist, set it as a new entry
