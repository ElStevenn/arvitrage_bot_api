...
class BitgetService:
    ...
    async def get_crypto_period(self, symbol):
        """Get funding rate, either 8k or 4h"""
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    if diference == datetime(hour=8):
                        return 8
                    elif diference == datetime(hour=4):
                        return 4
                    else:
                        raise ValueError("There was an unkowed diference..")
...
async def main_testing():
    ...
    res = await bitget_layer.get_crypto_period("BIGTIMEUSDT")
