...
class BitgetService:
    ...
    async def get_historical_funding_rate(self, symbol: str):
        ...
        for page_number in [page * 100 for page in range(self.max_pages)]:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(url, params=params) as response:
                        if response.status == 200:
                            ...
                            # Convert NumPy array to list of Python native types for 
                            page_array = jsonable_encoder(np_data.tolist())
                            np.append(final_result, page_array)
                        else:
