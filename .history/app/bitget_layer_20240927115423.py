...
class BitgetService:
    ...
    async def get_all_cryptos(self):
        ...
        async def get_(self, symbol: str, granularity, start_time: int = None, end_time: int = None) -> np.ndarray:
                ...
                while True:
                    async with session.get(base_url, params=params) as response:
                        if response.status == 200:
                            ...
                            # If last_timestamp reaches or exceeds end_time, stop fetching data
...
async def main_testing():
    ...
    end_time = int(datetime(2024, 9, 11).timestamp() * 1000)
