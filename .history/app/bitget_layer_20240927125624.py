...
class BitgetService:
    ...
    def calculate_api_calls(start_time: int, end_time: int, granularity_ms: int):
    ...
    async def get_candlestick_chart(self, symbol: str, granularity: str, start_time: int = None, end_time: int = None) -> np.ndarray:
            ...
            api_calls = self.calculate_api_calls(start_time, end_time, granularity_ms); print(f"Needed api calls -> {api_calls}")
            ...
            async with aiohttp.ClientSession() as session:
                for i, call in enumerate(api_calls):
                    ...
                    async with session.get(base_url, params=params) as response:
                        if response.status == 200:
                            result = await response.json(); print("api, respult -> ", result)
                            data = result.get("data", [])
...
async def main_testing():
    ...
    end_time = int(datetime(2024, 9, 8).timestamp() * 1000)  # Later time
