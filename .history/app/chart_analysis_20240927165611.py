...
class FundingRateChart:
    ...
    async def get_8h_variation(self, period: int):
        ...
        df = pd.DataFrame(candle_stick_data, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume', 'notional', 'datetime', 'wtf'])
...
async def main_testing():
    ...
    period = int(datetime(2024, 9, 11).timestamp() * 1000)

    res = await chart_analysis.get_8h_variation(period)
    print(res)
