...
class RedisService:
    ...
    def add_new_crypto(self, symbol: str, whole_analysis_until_now: List[dict], period: Literal['8h', '4h']):
        ...
        if crypto_list:
            try:
                # Load the list as JSON if it exists and is valid
                crypto_list = json.loads(crypto_list)
            except json.JSONDecodeError:
                # In case of a decoding error (improper format), reset the list
                crypto_list = []
        else:
...
if __name__ == "__main__":
    ...
    # pprint(crypto)
    # print(redis_service.delete_everything())